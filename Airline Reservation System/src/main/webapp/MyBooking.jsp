...
%>
  	<tr>
  		...
  		<td><a href="ShowBook?&flightid=<%=la.getFlightid()%>&flightname=<%=la.getFlightname()%>&from=<%=la.getFrom()%>&to=<%=la.getTo()%>&date=<%=la.getDate()%>&deptime=<%=la.getDeptime()%>&pnrno=<%=la.getPnrno()%>">Show</a>
  		</td>
  	</tr>
<%
  		}
