...
<table width="770" border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF">
 ...
 %>
  	<tr>
  		...
  		<td align="center"><%=la.getFltype() %></td>
  		<td align="center"><%=la.getFrom() %></td>
  		<td align="center"><%=la.getTo() %></td>
  		<td align="center"><%=la.getDate() %></td>
  		<td align="center"><%=la.getDeptime() %></td>
  		<td><a href="ViewFlight?&flightid=<%=la.getFlightid()%>&flightname=<%=la.getFlightname()%>&fltype=<%=la.getFltype()%>&from=<%=la.getFrom()%>&to=<%=la.getTo()%>&date=<%=la.getDate()%>&deptime=<%=la.getDeptime()%>">Show</a></td>
  	</tr>
 <%
  		}
 %>
 ...
 <%
  		...
  		{
  		l.setMsg("Your have no flight.");
  		
  %>
  
  <font color="green" size="5" face="Monotype Corsiva"><%= l.getMsg()%></font>
  <%
  }
   %>
